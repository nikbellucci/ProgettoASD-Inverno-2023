...
/**
  * L'intero programma seguirá la complessitá dominante che si presenta cioè quella della ricerca mediante l'algoritmo di Dijkstra con coda di priorità O((n + m) * log(n)).
  */
public class Esercizio4 {
    ...
    public static void main(String[] args) {
        if (args.length != 1) {
            System.out.println("Usage: java Esercizio4 <input_file>");
            System.exit(1);
        }
        FILENAME = args[0];
...
//        System.out.println("#####################################################");
//        for (NodeAtlanta NodeAtlanta: nodes) {
//            System.out.println(NodeAtlanta.getUniqueId() + ":<" + NodeAtlanta.getNameId() + ", (" + NodeAtlanta.getLongitude() + ", " + NodeAtlanta.getLatitude() + ")" + ">");
//        }
//        for (Link link: links) {
...
//        System.out.println("#####################################################");
        ...
        NodeAtlanta source = nodes.get(3);
        NodeAtlanta destination = nodes.get(11);
        long startTime = System.currentTimeMillis();
        List<List<Integer>> paths = findShortestPaths(weights, source.getUniqueId(), destination.getUniqueId());
        ...
        System.out.println("All paths with the same minimum cost from " + source.getNameId() + " to " + destination.getNameId() + ": ");
    ...
    public static void indexNodesInLinks(List<NodeAtlanta> nodes, List<Link> links) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            NodeAtlanta NodeAtlanta = nodes.get(i);
            nodeIndexMap.put(NodeAtlanta.getNameId(), NodeAtlanta.getUniqueId());
        }
    ...
    /**
     * L' algoritmo di Dijkstra con coda di priorità impiega O((n + m) * log(n)), dove n è il numero totale di nodi e m è il numero totale di archi.
     * @param graph
     * @param source
     * @param destination
     * @return
    ...
    public static List<List<Integer>> findShortestPaths(double[][] graph, int source, int destination) {
        ...
        PriorityQueue<NodeAtlanta> minHeap = new PriorityQueue<>();
        boolean[] visited = new boolean[n];
        double[] distance = new double[n];
        Arrays.fill(distance, Double.POSITIVE_INFINITY);

        distance[source] = 0;
        minHeap.add(new NodeAtlanta(source, 0.0));

        while (!minHeap.isEmpty()) {
            NodeAtlanta currentNode = minHeap.poll();
            ...
            for (int neighbor = 0; neighbor < n; neighbor++) {
                if (graph[currentVertex][neighbor] > 0.0) {
                    ...
                    if (newDistance < distance[neighbor]) {
                        ...
                        minHeap.add(new NodeAtlanta(neighbor, newDistance));
                    }
                }
            }
        }
...
//class NodeAtlanta implements Comparable<NodeAtlanta> {
...
//
//    @Override
//    public int compareTo(NodeAtlanta other) {
